        ...
        public static void ProcessTouchInput(out Vector2 player1Velocity, out Vector2 player2Velocity)
        {
            ...
            foreach (GestureSample gestureSample in Gestures)
            {
                if (gestureSample.GestureType == GestureType.FreeDrag)
                {
                    ...
                    if (gestureSample.Position.X >= 0 && gestureSample.Position.X <= screenwidth / 2) player1Velocity.Y += gestureSample.Delta.Y;
                    if (gestureSample.Position.X >= screenwidth / 2 && gestureSample.Position.X <= screenwidth) player2Velocity.Y += gestureSample.Delta.Y;
                }
            }
        }
    }
